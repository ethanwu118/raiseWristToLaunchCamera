...
// TINNO BEGIN
// add by wenjun.bai for VBCNRUA-145 face id
import com.android.server.faceidentify.FaceIdUnlockService;
// TINNO END
//Ethan Wu [CHGAAES-818] s:implement for raise to take photo
import com.android.server.innoservice.RaiseTakePhotoService;
//Ethan Wu [CHGAAES-818] e:implement for raise to take photo
...
//Tinno:CJ some import {
import com.tinno.system.TinnoCustomFeature;
import com.android.server.am.TinnoPerformanceService;
import com.android.server.tinno.TinnoConfigService;
import com.android.server.tinno.TinnoMonitorService;
// }
...
public final class SystemServer {
    ...
    private void startOtherServices() {
        ...
        // Tinno:CJ for tinno config service {
        if(TinnoCustomFeature.TINNO_CONFIG_SERVICE){
            try {
                 Slog.i(TAG, "TinnoCustomFeature");
                 mSystemServiceManager.startService(TinnoConfigService.Lifecycle.class);
            } catch (Throwable e) {
                 Slog.w(TAG, "TinnoConfigService error",e);
            }
        }
        // }

		
        //add by CJ for tinno Performance Service {
        if(TinnoCustomFeature.ENABLE_APP_START_CONTROL){
            try {
                 Slog.i(TAG, "TinnoPerformanceService");
                 ServiceManager.addService(TinnoPerformanceService.ServiceName, new TinnoPerformanceService(context, mActivityManagerService));
            } catch (Throwable e) {
                 Slog.w(TAG, "TinnoPerformanceService error",e);
            }
        }
        //add by CJ for tinno Performance Service }        
        ...
        //TINNO_BEGIN
        //add by sunny.sang for tinno monitor cpu and gpu to debug performance issue
        try {
            if(TinnoCustomFeature.SYSTEM_ENHANCED) {
                try {
                     Slog.i(TAG, "In SystemServer, TinnoMonitorService add..");
                     ServiceManager.addService("TinnoMonitorService", new TinnoMonitorService(context));
                } catch (Throwable e) {
                     Slog.i(TAG, "In SystemServer, TinnoMonitorService add err..");
                }
            }
        }
        catch (Exception e) {
            Slog.w(TAG, "*******************TINNO add TinnoLoggerService got some exception ", e);
        }
        //add by sunny.sang for tinno logger
        //TINNO_END
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            if (!disableNonCoreServices) {
                ...
                // TINNO BEGIN
                // add by wenjun.bai for VBCNRUA-145 face id
                if (android.os.SystemProperties.getBoolean("ro.feature.faceidentify", false)) {
                    mSystemServiceManager.startService(FaceIdUnlockService.class);
                } else {
                   /* if (mPackageManager.hasSystemFeature(PackageManager.FEATURE_FINGERPRINT)) {
                        mSystemServiceManager.startService(FingerprintService.class);
                    }*/
                ...
                //Ethan Wu [CHGAAES-818] s:implement for raise to take photo
                if (android.os.SystemProperties.getBoolean("ro.feature.landscapeLaunchCamera", false)) {
                	mSystemServiceManager.startService(RaiseTakePhotoService.class);
                }
		//Ethan Wu [CHGAAES-818] e:implement for raise to take photo
